...
} from '../../../repo/field-builders/index.js';
...
export abstract class SqlGenerator {
    ...
    protected _buildSubquerySelectClause<T extends AnyModel>(
                    ...
                    if (sel.fieldType === 'aggregate-field') {
                        ...
                        if (!a.alias)
                            throw Errors.query.error(
                                `Aggregate '${a.func}' must have an alias in a subquery.`,
                            );
                        let inner =
                            a.field !== '*'
        ...
        const publicFields = [
            ...relatedSchemaCore.registry.fieldsByName.values(),
        ]
            .filter(
                (f) => f.persist !== false && !this.securedTypes.has(f.type),
            )
    ...
    private _buildSelectQuery<T extends AnyModel>(
        ...
        const values: any[] = [];
        return {
            text: this._buildSelectQueryText(sqb, values, { p: 1 }),
            values,
        };
    }
    ...
    private _buildSelectQueryText<T extends AnyModel>(
        ...
        if (!mainTableAlias) throw Errors.query.noTableContext();
            ...
            selectClause = sqb._selects
                .map((sel: any) => {
                    ...
                    if (sel.fieldType === 'aggregate-field') {
                        ...
                        if (!a.alias)
                            throw Errors.query.error(
                                `Aggregate '${a.func}' must have an alias.`,
                            );
                        const inner =
                            a.field !== '*'
        ...
        for (const island of joinIslands) {
            ...
            while (tablesAddedInPass) {
                ...
                for (const join of joinsForIsland) {
                    ...
                    if (newAlias) {
                        const onSql = this._buildConditionSql(
                            join.on,
                            values,
                            paramIndex,
                        );
