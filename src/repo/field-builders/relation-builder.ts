...
> = {
    [K in keyof RelationsOf<T> & string]: RelationsOf<T>[K] extends ToOneRelation<
      infer R extends AnyModel
    >
    ? ToOneRelationBuilder<R, T, K, K, [], TParentAlias>
    : RelationsOf<T>[K] extends ToManyRelation<
      infer R extends AnyModel
    >
...
>
  ...
  as<A extends string>(
    ...
    updateWhereAlias(
      newBuilder.internalSqb._wheres,
      this.alias,
      alias,
    );
  ...
  ): ToOneRelationBuilder<T, TParent, TName, TAlias, TNested, TParentAlias, S> {
    this.internalSqb._selects = selector(this.createFieldProxy(), {} as any);
    return this as unknown as ToOneRelationBuilder<T, TParent, TName, TAlias, TNested, TParentAlias, S>;
  }
  ...
  > {
    ...
    const includedRelations = selectedRelationBuilders.map((relBuilder) =>
      resolveInclude(relBuilder, this.relatedSchemaCore.collection, this.appCore, this.relatedSchemaCore),
    );
    ...
    return new Proxy(
      {},
      {
        get: (target: {}, prop: string | symbol) => {
          ...
          if (!relationMetadata)
            throw Errors.repo.relationNotFound(relationName, this.relatedSchemaCore.collection);
          ...
          if (!relatedSchemaCore)
            throw Errors.repo.relatedSchemaNotFound(relationName, relationMetadata.toSchema);
...
>
  extends BaseWhereBuilder<
    FilterProxy<T>,
    ToManyRelationBuilder<T, TParent, TName, TAlias, TNested, TParentAlias, TSelects>
  >
  implements
  IQueryBuilder,
  ...
  ): ToManyRelationBuilder<T, TParent, TName, TAlias, TNested, TParentAlias, S> {
    this.internalSqb._selects = selector(this.createFieldProxy(), {} as any);
    return this as unknown as ToManyRelationBuilder<T, TParent, TName, TAlias, TNested, TParentAlias, S>;
  }
  ...
  order(
    selector: (fields: SelectableFieldsProxy<T>) => SelectableField<T, keyof T>,
