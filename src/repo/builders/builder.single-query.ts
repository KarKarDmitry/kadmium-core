...
>
  ...
  >(
    ...
    const includedRelations = selectedRelationBuilders.map((relBuilder) =>
      resolveInclude(relBuilder, this.collectionName, this.appCore, this._schemaCore),
    );
    ...
    return new Proxy(
      {},
      {
        get: (target: {}, prop: string | symbol) => {
          ...
          if (!relationMetadata) {
            throw Errors.repo.relationNotFound(relationName, this.collectionName);
          }
  ...
  update(data: Partial<T>) {
    ...
    const go = async (): Promise<T[]> => {
      ...
      if (modelOp !== "update") {
        throw Errors.query.error(`Model beforeUpdate hook changed operation to "${modelOp}". This is not supported for update().`);
      }
      // 2. Feature beforeUpdate hooks (infrastructure logic)
      const { data: processedData, operation } = await this._repo._runFeatureBeforeUpdate(modelData as Partial<T>);
      if (operation !== "update") {
        throw Errors.query.error(`beforeUpdate hook changed operation to "${operation}". This is not supported for update().`);
      }
      this._sqb._updateData = await this._repo._hashPasswordsInData(processedData as Partial<T>);
      const results = (await this.go()) as any;

      // 1. Model afterUpdate hooks (user business logic)
      let updatedResults = await this._repo._runModelAfterUpdate(results as T[]);
  ...
  delete() {
    ...
    const go = async () => {
      ...
      if (modelOp !== "delete") {
        throw Errors.query.error(`Model beforeDelete hook changed operation to "${modelOp}". This is not supported for delete().`);
      }
  ...
  private async go(): Promise<Partial<T>[]> {
    ...
    if (this.sqb._operation === "select") {
      const modelResults = await this._repo._runModelAfterRead(results as T[]);
