...
>
	...
	constructor(
		...
		const whereProxy = createMultiQueryWhereProxy<T>(schemaCores, sqb, appCore);
					...
					return new Proxy(
						{},
						{
							get: (target2: {}, prop: string | symbol) => {
								...
								const relationName = prop as keyof RelationsOf<any> & string;
								const relationMetadata = this.appCore.relationMap.get(
									`${collectionName}:${relationName}`,
								);

								if (!relationMetadata) {
									throw Errors.repo.relationNotFound(relationName, collectionName);
								}
	...
	>(
		...
		const includedRelations = selectedRelationBuilders.map((relBuilder) => {
			...
			if (!parentSchemaCore) {
				throw Errors.query.error(`Schema core not found for alias "${relBuilder.parentAlias}" during include setup.`, { alias: relBuilder.parentAlias });
			}
	...
	public select<const S extends readonly AnySelectable[]>(
		...
		if (!selected || selected.length === 0) {
			...
			throw Errors.query.error("A select call on a multi-query must select at least one field.");
		}
					...
					return new Proxy(
						{},
						{
							get: (target2: {}, fieldName: string | symbol) => {
								return new SelectableField<Model, keyof Model, A>({
									tableAlias: alias,
