...
export class KadmiumRepo<T extends AnyModel> {
    ...
    async _runModelBeforeCreate(
        ...
        const ctx = new ModelHookContext<T>(
            'create',
            this,
            this.schemaCore,
            data,
     ...
     */
    public async _runFeatureBeforeRead(
        alias?: string,
        ...
        const runner = {
            go: async (): Promise<T | T[]> => {
                ...
                const processSingleItem = async (item: Partial<T>) => {
                    const newItem = { ...item };
                    if (
                        pkField.db_type === 'uuid' &&
                        !newItem[pkField.name as keyof T]
                    ) {
                ...
                if (Array.isArray(data)) {
                    ...
                    for (let i = 0; i < results.length; i++) {
                        ...
                        results[i] = await this._runModelAfterCreate(
                            results[i] as T,
                        );
    ...
    >(
        ...
        T,
        R,
        FlatFinalResult<S> & IncludeResult<Public<T>, R>
    >;
    select<
        const S extends readonly AnySelectable[],
        R extends IRelationBuilder[],
    ...
    @Profiler.Profile(import.meta.url)
    async transaction<R>(
        callback: (tx: ITransaction) => Promise<R>,
    ): Promise<R> {
