...
export class FieldValidator<T> {
	...
	get req(): this {
		return new FieldValidator(this._type, true, this._min, this._max) as this;
	}

	get opt(): FieldValidator<T | undefined> {
		return new FieldValidator(this._type, false, this._min, this._max) as any;
	}

	min(n: number): this {
		return new FieldValidator(this._type, this._required, n, this._max) as this;
	}

	max(n: number): this {
		return new FieldValidator(this._type, this._required, this._min, n) as this;
	}
...
export const v = {
	...
	object<S extends Record<string, FieldValidator<any> | ObjectValidator<any>>>(
		schema: S,
	): ObjectValidator<S> {
...
export type InferValStruct<T> = T extends FieldValidator<infer R>
	? R
