...
export const Errors = {
  schema: {
    notFound: (collection: string, cause?: Error) => new SchemaNotFoundError(collection, cause),
    duplicateField: (field: string, schema: string, cause?: Error) => new DuplicateFieldError(field, schema, cause),
    duplicateSection: (key: string, location: string, cause?: Error) => new DuplicateSectionError(key, location, cause),
    modelMismatch: (modelName: string, modelCollection: string, schemaCollection: string, cause?: Error) => new ModelMismatchError(modelName, modelCollection, schemaCollection, cause),
    missingCollection: (cause?: Error) => new MissingCollectionError(cause),
    missingPrimary: (collection: string, cause?: Error) => new MissingPrimaryError(collection, cause),
    missingPrimaryName: (collection: string, cause?: Error) => new MissingPrimaryNameError(collection, cause),
    missingPrimaryDbType: (collection: string, cause?: Error) => new MissingPrimaryDbTypeError(collection, cause),
    missingForm: (collection: string, cause?: Error) => new MissingFormError(collection, cause),
  },
  repo: {
    notFound: (schema: string, cause?: Error) => new RepoNotFoundError(schema, cause),
    relationNotFound: (relation: string, schema: string, cause?: Error) => new RelationNotFoundError(relation, schema, cause),
    relatedSchemaNotFound: (relation: string, toSchema: string, cause?: Error) => new RelatedSchemaNotFoundError(relation, toSchema, cause),
  },
  query: {
    error: (message: string, context?: Record<string, any>, cause?: Error) => new QueryError(message, context, cause),
    unsupportedOp: (op: string, cause?: Error) => new UnsupportedOperationError(op, cause),
    noTableContext: (cause?: Error) => new NoTableContextError(cause),
    updateNoData: (cause?: Error) => new UpdateNoDataError(cause),
    singleTableOnly: (op: "UPDATE" | "DELETE", cause?: Error) => new SingleTableOnlyError(op, cause),
    notFound: (collection: string, cause?: Error) => new RecordNotFoundError(collection, cause),
    invalidAlias: (alias: string, cause?: Error) => new InvalidAliasError(alias, cause),
  ...
  validation: {
    failed: (details: { path: string; message: string }[], cause?: Error) => new ValidationError(details, cause),
    adapterNotCompiled: (cause?: Error) => new AdapterNotCompiledError(cause),
  },
  feature: {
    abort: (operation: string, reason?: string, cause?: Error) => new FeatureAbortError(operation, reason, cause),
  },
  config: {
