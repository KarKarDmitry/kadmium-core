...
export class DbMutator {
    ...
    async applyMigrations(
        txAdapter: TransactionalDbAdapter,
    ): Promise<string[]> {
        ...
        const hasCreateTable = diff.operations.some(
            (op) => op.type === 'create-table',
        );
        ...
        const tableOps = diff.operations.filter(
            (op) => op.type === 'create-table',
        );
        ...
        const createdTableNames = new Set(
            tableOps.map((op) => (op as CreateTableOp).table),
        );
        const filteredOps = secondDiff.operations.filter((op) => {
            if (
                op.type === 'create-table' &&
                createdTableNames.has((op as CreateTableOp).table)
            ) {
                return false;
            }
            return true;
        ...
        const filteredDiff: DiffResult = {
            ...
            summary: {
                ...secondDiff.summary,
                addedTables: filteredOps.filter(
                    (o) => o.type === 'create-table',
                ).length,
            },
        };
