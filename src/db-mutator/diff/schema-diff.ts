...
import {
    Ref_OPT,
    NormalizedInputField_OPT,
} from '../../schema/types/fields.js';
 ...
 */
function fieldTypeToPgDataType(
    field: DbColumn,
    ...
    switch (type) {
        ...
        case 'ref': {
            ...
            const refFieldName = (f as any).ref || f.ref;
            const targetSchema = appCore.schemas.find(
                (s) => s.collection === refFieldName,
            );
            if (targetSchema) {
                const pkType = targetSchema.normalized.primary.db_type;
                if (pkType === 'uuid') return 'uuid';
                if (pkType === 'string') return 'character varying';
                return 'integer';
            }
...
export class SchemaDiff {
    ...
    computeDiff(): DiffResult {
        ...
        const schemaTables = new Set(
            this.appCore.schemas.map((s) => s.collection),
        );
        ...
        for (const schemaCore of this.appCore.schemas) {
            ...
            if (!dbTables.has(tableName)) continue; // handled above
            ...
            const allInputFields = [
                ...
                (f): f is NormalizedInputField_OPT =>
                    (f as any).displayOnly !== true,
            );

            for (const field of allInputFields) {
                if ((field as any).persist === false) continue;
                ...
                if (!dbCol) {
                    ...
                    const normalizedExpected =
                        this._normalizePgType(expectedType);
                    const normalizedActual = this._normalizePgType(
                        dbCol.dataType,
                    );
                    ...
                    const expectedNullable =
                        !field.required && field.db?.nullable !== false;
            ...
            for (const dbCol of dbColumns) {
                const schemaField = allInputFields.find(
                    (f) => f.name === dbCol.name,
                );
                if (!schemaField) {
                    operations.push({
    ...
    checkHealth(): HealthCheckResult {
        ...
        if (diff.summary.addedTables > 0) {
            issues.push(
                `${diff.summary.addedTables} table(s) missing from database`,
            );
        }
        ...
        if (diff.summary.droppedColumns > 0) {
            issues.push(
                `${diff.summary.droppedColumns} extra column(s) in database`,
            );
        }
        ...
        if (diff.summary.addedForeignKeys > 0) {
            issues.push(
                `${diff.summary.addedForeignKeys} foreign key(s) missing`,
            );
        }
        ...
        return {
            ...
            summary: {
                ...
                matchingTables:
                    this.appCore.schemas.length - diff.summary.addedTables,
            },
        };
    }
    ...
    private _schemaToColumns(schemaCore: SchemaCore): DbColumn[] {
        const fields = [
            schemaCore.normalized.primary,
            ...schemaCore.normalized.form.fields,
        ];
        return fields
            .filter(
                (f) =>
                    (f as any).persist !== false &&
                    (f as any).displayOnly !== true,
            )
            .map((f) => this._fieldToColumn(f, schemaCore));
    }

    private _fieldToColumn(field: any, schemaCore: SchemaCore): DbColumn {
        ...
        const autoIncrement =
            field.type === 'primary' && field.auto_increment === true;
    ...
    private _expectedForeignKeys(schemaCore: SchemaCore): DbForeignKey[] {
        const fks: DbForeignKey[] = [];
        const tableName = schemaCore.collection;

        const inputFields = schemaCore.normalized.form.fields.filter(
            (f): f is NormalizedInputField_OPT =>
                (f as any).displayOnly !== true,
        );
        ...
        for (const field of inputFields) {
            ...
            const targetSchema = this.appCore.schemas.find(
                (s) => s.collection === refField.ref,
            );
     ...
     */
    private _normalizePgType(type: string): string {
