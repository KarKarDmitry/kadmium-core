...
import { RouteMiddleware } from "./adapter";
 ...
 */
export interface HandlerOptions<T extends AnyModel, P = unknown, Q = unknown, B = unknown> {
    /**
 ...
 */
export interface RouteDefinition<T extends AnyModel, P = unknown, Q = unknown, B = unknown> {
    /** HTTP method (lowercase): get, post, put, delete, patch */
    method: string;
    /** URL path with parameters (e.g., "/users/:id") */
    path: string;
    /** The route handler function */
    handler: (ctx: import("./context").ControllerContext<T>, req: ValidatedRequest<P, Q, B>) => Promise<any> | any;
