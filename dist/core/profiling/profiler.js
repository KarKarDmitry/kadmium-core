...
class Profiler {
     ...
     */
    static enable(state = true) {
        Profiler.isEnabled = state;
        console.log(`[Profiler] Collection is now ${Profiler.isEnabled ? "ENABLED" : "DISABLED"}.`);
    }
    /**
    ...
    static results() {
        ...
        try {
            if (fs.existsSync(PROFILE_OUTPUT_FILE)) {
                const fileContent = fs.readFileSync(PROFILE_OUTPUT_FILE, "utf-8");
                if (fileContent) {
            ...
            console.warn("[Profiler] Could not read or parse previous results file.", e);
        }
        // --- 2. Calculate current results ---
        const grouped = Profiler.records.reduce((acc, record) => {
        ...
        const currentResults = Object.values(grouped).map((group) => {
            const sum = group.timings.reduce((a, b) => a + b, 0);
            return {
                File: group.file,
                Method: group.method,
                "Total Calls": group.timings.length,
                "Avg (ms)": parseFloat((sum / group.timings.length).toFixed(4)),
                "Min (ms)": parseFloat(Math.min(...group.timings).toFixed(4)),
                "Max (ms)": parseFloat(Math.max(...group.timings).toFixed(4)),
                "Total (ms)": parseFloat(sum.toFixed(4)),
            };
        });
        // --- 3. Compare and display ---
        const colorizeDiff = (diffP) => {
            ...
            // Regression (Red)
            if (diffP > 5) { // Changed from 0.5 to 5
                const mag = diffP;
                if (mag >= 15)
        ...
        if (previousResults.length > 0) {
            const prevAvgMap = new Map(previousResults.map((r) => [r.Method, r["Avg (ms)"]]));
            const headers = [
                "Method",
                "Total Calls",
                "Avg (ms)",
        ...
        try {
            fs.writeFileSync(PROFILE_OUTPUT_FILE, JSON.stringify(currentResults, null, 2));
