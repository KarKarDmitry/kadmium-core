...
export class AppCoreSealTest extends UnitTest {
	...
	private test_configureBeforeSeal(): void {
		...
		if (app.app.port !== 4000) throw new Error("Port should be updated before seal");
	}
	...
	private test_configureAfterSeal(): void {
		...
		try {
			...
			if (!e.message.includes("sealed")) throw new Error("Expected 'sealed' error, got: " + e.message);
		}
	}
