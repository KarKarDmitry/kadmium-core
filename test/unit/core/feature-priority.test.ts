...
export class FeaturePriorityTest extends UnitTest {
	...
	private test_defaultPriority(): void {
		...
		if (sorted[0].name !== "A") throw new Error("Same priority should preserve order");
	}
	...
	private test_explicitPriority(): void {
		...
		const sorted = sortByPriority(features);
		if (sorted[0].name !== "SoftDelete") throw new Error("SoftDelete should be first");
		if (sorted[1].name !== "Audit") throw new Error("Audit should be second");
		if (sorted[2].name !== "Revisions") throw new Error("Revisions should be last");
	}
	...
	private test_unorderedInput(): void {
		...
		const sorted = sortByPriority(features);
		if (sorted[0].priority !== -100) throw new Error("First should be -100");
