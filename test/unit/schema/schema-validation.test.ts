...
export class SchemaValidationTest extends UnitTest {
    ...
    private test_missingCollection(): void {
        try {
            ...
            if (e.message === 'Should have thrown for empty collection')
                throw e;
        }
    }
    ...
    private test_missingPrimary(): void {
        try {
            ...
            if (!e.message.includes('primary'))
                throw new Error('Expected primary error, got: ' + e.message);
        }
    }
    ...
    private test_missingPrimaryName(): void {
        try {
            ...
            if (e.message === 'Should have thrown for missing primary name')
                throw e;
            if (!e.message.includes('name'))
                throw new Error('Expected primary name error');
        }
    }
    ...
    private test_missingPrimaryDbType(): void {
        try {
            ...
            if (e.message === 'Should have thrown for missing primary db_type')
                throw e;
            if (!e.message.includes('db_type'))
                throw new Error('Expected db_type error');
        }
    }
