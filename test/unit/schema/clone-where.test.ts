...
export class CloneWhereTest extends UnitTest {
	...
	private test_preservesFilterBuilderSymbol(): void {
		...
		const original: WhereGroup = {
			op: "AND",
			conditions: [
				{ field: "name", op: "=", value: filterBuilderMock } as WhereCondition,
			],
		};
	...
	private test_preservesQueryBuilderSymbol(): void {
		...
		const original: WhereGroup = {
			op: "AND",
			conditions: [
				{ field: "id", op: "IN", value: queryBuilderMock } as WhereCondition,
			],
		};
	...
	private test_nestedGroups(): void {
		...
		// Проверка структуры
		if (cloned.op !== "AND") throw new Error("nested: top-level op changed");
		if (cloned.conditions.length !== 2) throw new Error("nested: conditions count changed");

		const nestedGroup = cloned.conditions[1] as WhereGroup;
		if (nestedGroup.op !== "OR") throw new Error("nested: nested op changed");
		if (nestedGroup.conditions.length !== 2) throw new Error("nested: nested conditions count changed");
		...
		if ((originalNested.conditions[0] as WhereCondition).value === 999) {
			throw new Error("nested: clone mutation affected original nested condition");
		}
	}
	...
	private test_emptyGroup(): void {
		...
		if (cloned.conditions.length !== 0) throw new Error("empty: conditions not empty");
	}
}
