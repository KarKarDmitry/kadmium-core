...
export class HookFilterProxyTest extends UnitTest {
    ...
    private test_eq(): void {
        ...
        f.title.eq('Hello');
        if ((wheres[0].op as string) !== '=')
            throw new Error('eq should use =');
    ...
    private test_in(): void {
        ...
        f.title.in(['A', 'B', 'C']);
        if ((wheres[0].op as string) !== 'IN')
            throw new Error('in should use IN');
    ...
    private test_like(): void {
        ...
        f.title.like('%test%');
        if ((wheres[0].op as string) !== 'LIKE')
            throw new Error('like should use LIKE');
        if (wheres[0].value !== '%test%')
            throw new Error('like value mismatch');
    }
    ...
    private test_getters_returnProxy(): void {
        ...
        f.title.null;
        if (wheres.length !== 1)
            throw new Error('.null should push 1 condition');
    }
}
