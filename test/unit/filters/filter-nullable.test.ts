...
export class NullableFilterTest extends UnitTest {
	...
	private test_null(): void {
		...
		if ((cond as any).value !== null) throw new Error(".null value should be null");
	}

	private test_notNull(): void {
		const cond = this.createBuilder().notNull;
		if (!cond) throw new Error(".notNull should return a condition");
		if ((cond as any).op !== "IS NOT") throw new Error(".notNull should use IS NOT");
		if ((cond as any).value !== null) throw new Error(".notNull value should be null");
	}

	private test_nullReturnsWhereCondition(): void {
		const cond = this.createBuilder().null;
		if (typeof cond !== "object") throw new Error(".null should return WhereCondition object");
		if (!(cond as any).field) throw new Error("Condition should have field");
	}
}
