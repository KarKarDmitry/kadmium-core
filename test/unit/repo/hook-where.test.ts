...
export class HookWhereTest extends UnitTest {
	...
	private test_basicEq(): void {
		...
		if (wheres[0].field !== "name") throw new Error("Expected field 'name'");
		if (wheres[0].op !== "=") throw new Error("Expected op '='");
		if (wheres[0].value !== "Alice") throw new Error("Expected value 'Alice'");
	}
	...
	private test_stringMethods(): void {
		...
		if ((wheres[0].op as string) !== "LIKE" || wheres[0].value !== "%active%") throw new Error("contains failed");
	...
	private test_numberMethods(): void {
		...
		wheres.length = 0;
		f.age.between(10, 20);
		if ((wheres[0].op as string) !== "BETWEEN") throw new Error("between failed");
		// Hook proxy stores value as array [min, max], real builder stores "10 AND 20" string
		const vals = wheres[0].value as number[];
		if (vals[0] !== 10 || vals[1] !== 20) throw new Error(`between values mismatch, got: ${JSON.stringify(vals)}`);
	}
	...
	private test_nullMethods(): void {
		...
		f.status.null;
		if (wheres.length !== 1) throw new Error("Expected 1 condition for .null");
		if ((wheres[0].op as string) !== "IS") throw new Error(".null should use IS");

		wheres.length = 0;
		f.status.notNull;
		if ((wheres[0].op as string) !== "IS NOT") throw new Error(".notNull should use IS NOT");
	}
}
