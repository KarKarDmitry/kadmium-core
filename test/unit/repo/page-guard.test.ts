...
export class PageGuardTest extends UnitTest {
    ...
    private test_negativePage(): void {
        ...
        if (result.offset !== 0)
            throw new Error('Negative page should map to page 1 (offset 0)');
    }

    private test_zeroSize(): void {
        const result = this.computePage(1, 0);
        if (result.limit < 1)
            throw new Error('Size 0 should result in limit 1');
    ...
    private test_negativeSize(): void {
        ...
        if (result.limit !== 1)
            throw new Error('Negative size should result in limit 1');
    }
    ...
    private test_normalPagination(): void {
        ...
        if (p2.limit !== 10 || p2.offset !== 10)
            throw new Error('Page 2 wrong');
        if (p3.limit !== 10 || p3.offset !== 20)
            throw new Error('Page 3 wrong');
    }
}
