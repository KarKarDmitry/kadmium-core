...
export class CreateManyKeysTest extends UnitTest {
    ...
    private test_differentKeys(): void {
        ...
        if (keys.length !== 3)
            throw new Error(
                `Expected 3 keys, got ${keys.length}: ${JSON.stringify(keys)}`,
            );
        if (!keys.includes('name'))
            throw new Error("Should include 'name' from first element");
        if (!keys.includes('email'))
            throw new Error("Should include 'email' from second element");
    }
    ...
    private test_missingFieldsBecomeNull(): void {
        ...
        if (values[3] !== null)
            throw new Error("Expected null for missing 'name' at position 3");
    }
}
