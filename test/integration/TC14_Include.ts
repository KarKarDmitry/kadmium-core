...
export class TC14_Include extends IntegrationTestCase {
	...
	protected async runImpl(): Promise<void> {
		...
		const res7 = await userRepo
			...
			.first()
			.go();
		const post1WithComments = res7?.posts.find((p) => p.id === testPost1.id);
		...
		const res8 = await userRepo
			...
			.first((u) => [u.id])
			.go();
		if (res8?.dept?.name !== "Include Test Dept" || res8.id !== testUser.id) {
			throw new Error(`[FAIL 14.8] Include with selector and alias failed.`);
		}
