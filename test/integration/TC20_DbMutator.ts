...
export class TC20_DbMutator extends IntegrationTestCase {
	...
	private async t_inspectTables(inspector: DbInspector): Promise<void> {
		...
		if (!dbSchema || !Array.isArray(dbSchema.tables) || dbSchema.tables.length === 0) {
	...
	private async t_addColumnsViaMigration(
		...
		console.log("   - Columns created with tables (no separate add needed)");
	}
	...
	private async t_addFkViaMigration(
		...
		const fkOps = diffResult.operations.filter(
			(op) =>
				op.type === "add-foreign-key" || op.type === "add-index",
		);
		...
		if (!hasAuthorFk) {
			throw new Error("tc20_post.author_id FK was not created after migration");
		}
	...
	private async t_healthCheckAfterMigrations(
		...
		if (!health.isHealthy) {
			...
			const typeOps = fullDiff.operations.filter(
				(op) => op.type === "alter-type" || op.type === "alter-nullable",
			);
		...
		const criticalIssues = health.issues.filter(
			(issue) =>
				!issue.includes("type/nullability"),
		);
		...
		console.log("   - Health check passes after migrations (minor type diffs OK)");
	}

	/* ── Helper: build AppCore with test schemas ── */
	private _buildTestAppCore(dbAdapter: any): any {
		// Import schemas directly
		const { tc20_userSchema } = require("../../src/test-schemas/tc20-user.schema");
		const { tc20_postSchema } = require("../../src/test-schemas/tc20-post.schema");
