...
export class TC6_BulkCreate extends IntegrationTestCase {
	...
	protected async runImpl(): Promise<void> {
		...
		const createdUsers = (await userRepo.create(bulkUsersData).go()) as User[];

		if (!Array.isArray(createdUsers) || createdUsers.length !== 2) {
			throw new Error(
				`Expected to create 2 users, but got ${Array.isArray(createdUsers) ? createdUsers.length : "not an array"
				}.`,
			);
		}
