...
export class TC19_Transactions extends IntegrationTestCase {
	...
	private async testRepoMultiRepoTransaction(): Promise<void> {
		...
		if (!foundUser || !foundPost) {
			throw new Error("Multi-repo transaction did not commit both records");
		}
	...
	private async testControllerTransactionalRoute(): Promise<void> {
		...
		const res = await httpPost(`http://localhost:${this.port}/ctrl/tx-create`, {
			first_name: "CtrlTx",
			last_name: "Test",
			email,
			username,
		...
		if (!found || found.first_name !== "CtrlTx") {
			throw new Error("User from transactional controller was not committed");
		}
	...
	private async testControllerRollbackOnError(): Promise<void> {
		...
		const res = await httpPost(`http://localhost:${this.port}/ctrl/tx-fail`, {
