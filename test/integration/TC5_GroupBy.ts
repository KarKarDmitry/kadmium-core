...
export class TC5_GroupBy extends IntegrationTestCase {
    ...
    protected async runImpl(): Promise<void> {
        ...
        if (!aliceData || aliceData.post_count != 2) {
            throw new Error(
                `Alice should have 2 posts, but found ${
                    aliceData?.post_count ?? 'undefined'
                }.`,
            );
        }

        const bobData = postsPerUser.find(
            (row: any) => row.u.first_name === 'Bob',
        );
        if (!bobData || bobData.post_count != 1) {
            throw new Error(
                `Bob should have 1 post, but found ${
                    bobData?.post_count ?? 'undefined'
                }.`,
            );
        }
    }
