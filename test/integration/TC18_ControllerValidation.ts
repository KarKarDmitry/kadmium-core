...
export class TC18_ControllerValidation extends IntegrationTestCase {
    ...
    private async t_validBodyPassThrough(): Promise<void> {
        ...
        console.log(
            '   - Valid body with extra fields creates record correctly',
        );
    ...
    private async t_missingRequiredBody(): Promise<void> {
        ...
        if (b.message.includes('first_name') && b.message.includes('email')) {
            console.log(
                `   - Missing required body fields → 400: "${b.message}"`,
            );
        } else {
            throw new Error(
    ...
    private async t_stringMinValidation(): Promise<void> {
        ...
        if (!b.message)
            throw new Error(
                '400 should include error message for min validation',
            );
    ...
    private async t_emailFormatValidation(): Promise<void> {
        ...
        if (res.status !== 400)
            throw new Error(
                `Invalid email should return 400, got ${res.status}`,
            );
        const b = res.body as { message?: string };
    ...
    private async t_noValidationStillWorks(): Promise<void> {
        ...
        console.log(
            '   - Route without validation works (backward compatible)',
        );
    }
    ...
    private async t_paramsInvalidUuid(): Promise<void> {
        ...
        const { status } = await this.jget(
            this.url('/val/users/nonexistent-id'),
        );
        if (status === 400)
            throw new Error(
                `Param 'nonexistent-id' should pass string validation`,
            );
    ...
    private async t_queryValidation(): Promise<void> {
        ...
        const d = body.data;
        if (d.search !== 'hello')
            throw new Error(`Query 'q' should be 'hello'`);
        if (d.limit !== 10)
            throw new Error(
                `Query 'limit' should be coerced to number 10, got ${d.limit}`,
            );
        console.log(
            '   - Valid query params pass through (string→number coerced)',
        );
    }
    ...
    private async t_queryInvalidNumber(): Promise<void> {
        ...
        if (status !== 400)
            throw new Error(
                `Non-coercible number should return 400, got ${status}`,
            );
    ...
    private async t_optionalFields(): Promise<void> {
        ...
        if (res.status !== 200)
            throw new Error(
                `Boolean query param should pass, got ${res.status}`,
            );
        const body = JSON.parse(res.body);
        const d = body.data;
        if (d.active !== true)
            throw new Error(
                `'true' should coerce to boolean true, got ${d.active}`,
            );
    ...
    private async t_numberRangeValidation(): Promise<void> {
        ...
        if (resNeg.status !== 400)
            throw new Error(
                `Negative count should return 400, got ${resNeg.status}`,
            );
        ...
        if (resHigh.status !== 400)
            throw new Error(
                `count > 1000 should return 400, got ${resHigh.status}`,
            );
    ...
    private async t_maxValidation(): Promise<void> {
        ...
        if (res.status !== 400)
            throw new Error(
                `Name > 50 chars should return 400, got ${res.status}`,
            );
    ...
    private async t_extraFieldsPreservedThroughValidation(): Promise<void> {
        ...
        if (res.status !== 200)
            throw new Error(
                `Extra fields should pass ValStruct, got ${res.status}`,
            );
        ...
        console.log(
            '   - Extra fields preserved through validation gatekeeper',
        );
