...
export class TC22_Features_Extended extends IntegrationTestCase {
	...
	protected async runImpl(): Promise<void> {
		...
		if (!commentSchemaCore.features.some((f) => f instanceof SoftDeleteFeature))
		...
		console.log("1. SoftDelete: deleted_at field added to comment registry.");
		...
		if (rawResult.length === 0)
			throw new Error("2. Comment should still exist in DB (soft delete).");
		if (!rawResult[0]?.deleted_at)
			throw new Error("2. Comment should have deleted_at after soft delete.");
		...
		try {
			...
			if (res1.statusCode !== 200)
				throw new Error(`4. GET /revisions failed with ${res1.statusCode}`);
			const body1 = JSON.parse(res1.body);
			if (body1.type !== "data" || body1.data.length === 0)
				throw new Error("4. No revisions found after department create.");
			console.log(
