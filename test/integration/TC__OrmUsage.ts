...
export class TC__OrmUsage extends IntegrationTestCase {
	...
	protected async runImpl(): Promise<void> {
		...
		const newUser = await userRepo
			...
			.go();
		console.log(`1. Created User: ${newUser.first_name} (ID: ${newUser.id})`);
		...
		console.log(`3. Selected all public users. Count: ${usersPublic.length}`);

		// --- 4. Select specific fields with alias ---
		const userNames = await userRepo
			.select((u) => [u.first_name.as("firstName"), u.last_name.as("lastName")])
			.go();
		console.log(
		...
		const specificUser = await userRepo
			...
			.first()
			.go();
		if (!specificUser) throw new Error("5. User not found with where clause.");
		...
		console.log(`12. User updated first_name to: ${updatedUser.first_name}`);
		...
		if (
			...
			throw new Error("20. Complex multi-table query with GROUP BY failed.");
		}
		console.log(
