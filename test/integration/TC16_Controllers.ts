...
export class TC16_Controllers extends IntegrationTestCase {
    ...
    private testDSL(): void {
        ...
        for (let i = 1; i <= 4; i++) {
            if (routes[i].options?.transactional !== true) {
                throw new Error(
                    `Route ${i} should default to transactional: true`,
                );
            }
        }
        ...
        if (customPost.options?.transactional !== false) {
            throw new Error(
                'Custom transactional option should override default',
            );
        }
    ...
    private async testHttpPipeline(): Promise<void> {
        ...
        const testCtrl = controller(User, [
            ...
            get('/ctrl/echo/:id', async (ctx) => ({
                id: ctx.request.params.id,
            })),
            get('/ctrl/query-test', async (ctx) => ({
                q: ctx.request.query.q,
            })),
            post('/ctrl/echo-body', async (ctx) => ({
        ...
        try {
            ...
            if (data.type !== 'data' || data.data.status !== 'ok') {
                throw new Error(
                    `/ctrl/health unexpected response: ${res.body}`,
                );
            }
            console.log('   - GET /ctrl/health passed');

            // GET /ctrl/echo/:id (path params)
            const echoRes = await httpGet(
                `http://localhost:${port}/ctrl/echo/42`,
            );
            ...
            if (queryData.data.q !== 'hello') {
                throw new Error(
                    `/ctrl/query-test unexpected: ${queryRes.body}`,
                );
            }
    ...
    private async testErrorHandler(): Promise<void> {
        ...
        try {
            ...
            if (errRes.status !== 500) {
                throw new Error(
                    `/ctrl/error should return 500, got ${errRes.status}`,
                );
            }
            ...
            if (
                ...
                throw new Error(
                    `/ctrl/error unexpected response: ${errRes.body}`,
                );
            }
            console.log('   - Error handler returns 500 with message');

            // GET /ctrl/not-found → 500
            const nfRes = await httpGet(
                `http://localhost:${port}/ctrl/not-found`,
            );
            if (nfRes.status !== 500) {
                throw new Error(
    ...
    private async testMiddleware(): Promise<void> {
        ...
        try {
            ...
            const res = await httpGet(
                `http://localhost:${port}/ctrl/middleware-ok`,
            );
            assertStatus(res, 200);
            ...
            const blockedRes = await httpGet(
                `http://localhost:${port}/ctrl/blocked`,
            );
    ...
    private async testPatchMethod(): Promise<void> {
        ...
        try {
            ...
            if (patchData.data.updated.first_name !== 'Updated') {
                throw new Error(
                    `/ctrl/partial-update unexpected: ${patchRes.body}`,
                );
            }
